...
a particular server.",
    ...
    "gpu_dds_workers": {
        ...
        "short_name": "GPU DDS workers",
        "short_name_japanese": "GPU DDS worker数",
        "hint": "Number of bounded concurrent DDS conversions inside the resident ASHelper process. M5 Max starts at 8; tune between 4 and 12 using the performance metrics.",
    },
    "gpu_batch_wait_ms": {
        "module": "TILE",
        "type": int,
    ...
    "enable_shared_memory_handoff": {
        ...
        "type": bool,
        "default": False,
        "short_name": "Shared-memory handoff",
        "short_name_japanese": "共有メモリ受け渡し",
        "hint": "Experimental: hand off eligible RGB8 imagery and DDS output through bounded shared memory. SSD cache and transaction activation remain the source of truth.\n\n日本語: 対応するRGB8画像とDDS出力を上限付き共有メモリで受け渡します。SSDキャッシュとtransactionの有効化が正本です。",
    },
    "shared_memory_budget_gb": {
        ...
        "short_name": "Shared-memory budget (GiB)",
        "short_name_japanese": "共有メモリ予算(GiB)",
        "hint": "Shared-memory budget per tile. Zero selects eight percent of physical memory, clamped to 2--8 GiB.",
    },
    ...
    "upscale_scope": {
        ...
        "type": str,
        "default": "all",
        "values": ("none", "all", "airport"),
        "short_name": "Upscale Scope",
        "short_name_japanese": "アップスケール範囲",
        "hint": "Select which textures use the selected 2x upscale backend. 'airport' uses explicit airport high-resolution texture metadata; missing airport data safely disables upscaling.\n\n日本語: 選択した2倍アップスケール方式を適用するテクスチャ範囲です。「airport」は空港由来の高解像度テクスチャだけを対象にし、空港データがない場合は安全のため拡大を無効にします。",
    },
    ...
    "upscale_backend": {
        "module": "IMG",
        "type": str,
        "default": "metalfx_spatial",
        "values": ("none", "ci_lanczos", "metalfx_spatial", "tensorops"),
        "short_name": "Upscale Backend",
        "short_name_japanese": "アップスケール方式",
        "hint": "Select the 2x orthophoto upscaling backend. New or missing settings use MetalFX Spatial by default; existing explicit settings are preserved. Core Image Lanczos is the compatibility path; TensorOps uses an external FP8SR pack on macOS 27 or newer and falls back to MetalFX Spatial, then Core Image Lanczos, when required.\n\n日本語: オルソフォトの2倍拡大バックエンドを選択します。新規または未設定の場合はMetalFX Spatialを既定値として使い、既存の明示設定は保持します。Core Image Lanczosは互換経路、TensorOpsはmacOS 27以降で外部FP8SRパックを使い、必要時はMetalFX Spatial、さらにCore Image Lanczosの順にフォールバックします。",
    },
    "fp8_model_path": {
        "module": "IMG",
        "type": str,
