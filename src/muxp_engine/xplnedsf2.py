...
class XPLNEDSF:   
    def __init__(self, logname='__XPLNEDSF__', statusfunction = "stdout"):
        ...
        # Keep the original atom stream so vendor-specific atoms survive a
        # read/modify/write cycle.  The legacy implementation only skipped
        # unknown atoms, which could silently discard data from newer DSF
        # producers.
        self._AtomSequence_ = []
        self._UnknownAtoms_ = []
 ...
 ########### END of NEW function _packCMDS_() #################               
    ...
    def read(self, file):  
        ...
        with open(file, "rb") as f:    ##Open Tile as binary fily for reading
            ...
            while f.tell() < flength - 16: #read chunks until reaching last 16 bytes hash value
                atom_start = f.tell()
                while atom_stack and atom_start >= atom_stack[-1][1]:
                    atom_stack.pop()
                bytes = f.read(8)
                atomID, atomLength = unpack('<4sI',bytes)        
                atomID = atomID.decode("utf-8")
                parent = atom_stack[-1][0] if atom_stack else None
                ...
                if atomID in self._AtomOfAtoms_:  
                    ...
                    self._AtomSequence_.append({"id": atomID, "parent": parent, "super": True})
                    atom_stack.append((atomID, atom_start + atomLength))
                elif atomID in self._AtomList_:
                    bytes = f.read(atomLength-8)   ##Length includes 8 bytes header
                    self._AtomSequence_.append({"id": atomID, "parent": parent, "super": False})
                    ...
                    self._UnknownAtoms_.append({
                        "id": atomID,
                        "parent": parent,
                        "payload": bytes,
                    })
                    self._AtomSequence_.append({
                        "id": atomID,
                        "parent": parent,
                        "super": False,
                        "unknown": True,
                        "unknown_index": len(self._UnknownAtoms_) - 1,
                    })
    ...
    def write(self, file): #writes data to dsf file with according file-name
        ...
        if self._AtomSequence_:
            known_payloads = {}
            for atom_id in self._AtomList_:
                value = self._Atoms_.get(atom_id)
                if value is None:
                    continue
                known_payloads[atom_id] = list(value) if atom_id in self._MultiAtoms_ else [value]
            known_indices = {atom_id: 0 for atom_id in known_payloads}

            def emit_atom(atom_id, payload):
                data = pack('<4sI', atom_id.encode('utf-8'), len(payload) + 8) + payload
                m.update(data)
                f.write(data)
                self._updateProgress_(len(data))

            def emit_children(parent_id, sequence_index):
                chunks = []
                while sequence_index < len(self._AtomSequence_):
                    entry = self._AtomSequence_[sequence_index]
                    if entry.get("parent") != parent_id:
                        break
                    sequence_index += 1
                    atom_id = entry["id"]
                    if entry.get("super"):
                        child_bytes, sequence_index = emit_children(atom_id, sequence_index)
                        chunks.append(pack('<4sI', atom_id.encode('utf-8'), len(child_bytes) + 8) + child_bytes)
                    elif entry.get("unknown"):
                        chunks.append(pack('<4sI', atom_id.encode('utf-8'), len(self._UnknownAtoms_[entry["unknown_index"]]["payload"]) + 8) + self._UnknownAtoms_[entry["unknown_index"]]["payload"])
                    else:
                        values = known_payloads.get(atom_id, [])
                        index = known_indices.get(atom_id, 0)
                        if index < len(values):
                            payload = values[index]
                            known_indices[atom_id] = index + 1
                            chunks.append(pack('<4sI', atom_id.encode('utf-8'), len(payload) + 8) + payload)
                return b''.join(chunks), sequence_index

            with open(file, "w+b") as f:
                self._log_.info("Write now DSF in file: {}".format(file))
                header = pack('<8sI', 'XPLNEDSF'.encode('utf-8'), 1)
                m.update(header)
                f.write(header)
                body, _ = emit_children(None, 0)
                f.write(body)
                m.update(body)
                f.write(m.digest())
            self._log_.info("Finished writing dsf-file with original atom ordering.")
            return 0
